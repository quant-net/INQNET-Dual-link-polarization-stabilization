...
class PolarizationStabilization:
    ...
    def stabilization_tracking(self, init=True, live_plot=True):
        ...
        while time_elapsed < self.duration:
            ...
            if live_plot:
                ...
                self.ax.set_ylabel("Visibility")
                self.ax.set_title("Live Visibility Tracking")
                self.ax.plot(self.tracking_data[0, :self.tracking_iter], self.tracking_data[1, :self.tracking_iter], label="Bob H1", color=colors[0])
                self.ax.plot(self.tracking_data[0, :self.tracking_iter], self.tracking_data[2, :self.tracking_iter], label="Bob D2", color=colors[1])
                self.ax.plot(self.tracking_data[0, :self.tracking_iter], self.tracking_data[3, :self.tracking_iter], label="Bob H2", color=colors[2])
                self.ax.plot(self.tracking_data[0, :self.tracking_iter], self.tracking_data[4, :self.tracking_iter], label="Alice H1", color=colors[3])
                self.ax.plot(self.tracking_data[0, :self.tracking_iter], self.tracking_data[5, :self.tracking_iter], label="Alice D2", color=colors[4])
                self.ax.plot(self.tracking_data[0, :self.tracking_iter], self.tracking_data[6, :self.tracking_iter], label="Alice H2", color=colors[5])
    ...
    def save_plot(self, filename):
        ...
        if self.cleaned_tracking_data.size > 0:
            ...
            self.ax.plot(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[1, :], label="Bob H1", color=colors[0])
            self.ax.plot(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[2, :], label="Bob D2", color=colors[1])
            self.ax.plot(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[3, :], label="Bob H2", color=colors[2])
            self.ax.plot(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[4, :], label="Alice H1", color=colors[3])
            self.ax.plot(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[5, :], label="Alice D2", color=colors[4])
            self.ax.plot(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[6, :], label="Alice H2", color=colors[5])
...
if __name__ == "__main__":
    ...
    duration = 12 * 60 * 60 # amount of time to run the stabilization algorithm for
