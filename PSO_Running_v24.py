...
from GenericUser_v2_remote import GenericUser
...
class PSOParams:
    def __init__(self, Alice, Bob, Charlie, Measure_CostFunction, log_event, log_file, meas_device):
        ...
        self.threshold_cost2 = 0.023#0.025#
        self.threshold_cost_Alice = 0.045#0.045#
        self.channels12 = [1, 2]
        self.channels34 = [3, 4]
...
def OSW_operate(OSW, Alice_Switch_status, Bob_Switch_status, meas_device, channels, log_file, initCheck = False,
    ...
    while True:
        ...
        if initCheck:
            ...
            if abs(power_check / reference_power - 1) < 0.9*1e9: # TODO Most likely need to change this tolerance here
                success = True
                break
...
class PolarizationStabilization:
    def __init__(self, pso_params, duration, tracking_interval=30, visTol=0.015):
        ...
        self.ax.set_title("Live Visibility Tracking")
        plt.ion()
        plt.show()
    ...
    def initial_stabilization(self):
        ...
        self.only_check(already_checked=True)

    def only_check(self,already_checked=False):
        # NOTE Only check, no stablization.
        if not already_checked:
            self.log_event(self.log_file, "Checking Step 1")
            self.step1.check()
            self.step3.check_H1H2()
            self.log_event(self.log_file, "Checking Step 2")
            self.step2.check()
            self.log_event(self.log_file, "Checking Step 3")
            self.step3.check_H1D2()
            self.log_event(self.log_file, "Checking Step 4")
            self.step4.check()
        ...
        self.ax.set_title("Stabilization Tracking Plot")
        self.ax.scatter(self.tracking_data[0, :self.tracking_iter], self.tracking_data[1, :self.tracking_iter], label="Bob H1", color=colors[0])
        self.ax.scatter(self.tracking_data[0, :self.tracking_iter], self.tracking_data[2, :self.tracking_iter], label="Bob D2", color=colors[1])
        self.ax.scatter(self.tracking_data[0, :self.tracking_iter], self.tracking_data[3, :self.tracking_iter], label="Bob H2", color=colors[2])
        self.ax.scatter(self.tracking_data[0, :self.tracking_iter], self.tracking_data[4, :self.tracking_iter], label="Alice H1", color=colors[3])
        self.ax.scatter(self.tracking_data[0, :self.tracking_iter], self.tracking_data[5, :self.tracking_iter], label="Alice D2", color=colors[4])
        self.ax.scatter(self.tracking_data[0, :self.tracking_iter], self.tracking_data[6, :self.tracking_iter], label="Alice H2", color=colors[5])
        #self.ax.set_ylim(-0.01, 0.1)
    ...
    def check_H_stability(self):
        ...
        self.log_event(self.log_file, "Rechecking H polarization steps...") #TODO Maybe add self.stepX.check() for all steps?
    ...
    def check_D_stability(self):
        ...
        self.log_event(self.log_file, "Rechecking D polarization steps...") #TODO Maybe add self.stepX.check() for all steps?
        if (not self.step1.success or not self.step3.H1_success
    ...
    def stabilization_tracking(self, init=True, live_plot=True):
        ...
        while time_elapsed < self.duration:
            self.log_event(self.log_file, f"Stabilization Tracking Loop Iteration {self.tracking_iter}")
            ...
            if (time.perf_counter() - tic_H_tracking) >= self.SLEEP_TIME_H: # Add this as a separate method
                self.log_event(self.log_file, "Checking H stability")
                self.check_H_stability()
                tic_H_tracking = time.perf_counter()
            
            # Check D stability
            if (time.perf_counter() - tic_D_tracking)>= self.SLEEP_TIME_D:
                self.log_event(self.log_file, "Checking D stability")
                self.check_D_stability()
                tic_D_tracking = time.perf_counter()
    ...
    def clean_tracking_data(self):
        # Find columns where all values are -0.001
        mask = ~(self.tracking_data == -0.001).all(axis=0)

        # Keep only the columns that are not completely -0.001
        self.cleaned_tracking_data = self.tracking_data[:, mask]

    def save_tracking_data(self, filename):
        self.clean_tracking_data()
    ...
    def load_tracking_data(self, filename):
        ...
        self.f = loaded_data["raw_tracking_data"]
    ...
    def save_plot(self, filename):
        ...
        if self.cleaned_tracking_data.size > 0:
            ...
            self.ax.scatter(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[1, :], label="Bob H1", color=colors[0])
            self.ax.scatter(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[2, :], label="Bob D2", color=colors[1])
            self.ax.scatter(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[3, :], label="Bob H2", color=colors[2])
            self.ax.scatter(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[4, :], label="Alice H1", color=colors[3])
            self.ax.scatter(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[5, :], label="Alice D2", color=colors[4])
            self.ax.scatter(self.cleaned_tracking_data[0, :], self.cleaned_tracking_data[6, :], label="Alice H2", color=colors[5])
...
if __name__ == "__main__":
    ...
    for Rigol in Charlie.Rigols:
        Rigol.ch1_params = Rigol.set_channel_params(channel="1", burst_delay=47E-6,burst_cycles=1600, trigger_source="EXT")
...
#####################################################################
    try:
        ...
        for i in range(num_runs):
            ...
            ps.stabilization_tracking(live_plot=True)
        ...
        Charlie.Rigol2.control_channel(1, "OFF")
