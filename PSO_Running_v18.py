...
class PolarizationStabilization:
    ...
    def initial_stabilization(self):
        # NOTE Step.run() does not check whether self.failed_badly is True or False when self.success = False, only Step.check() does this
    ...
    def stabilization_tracking(self):
        ...
        while tocPS - ticPS < self.duration:
...
if __name__ == "__main__":
    ...
    duration = 60 * 60 *8 # amount of time to run the stabilization algorithm for
    ...
    Alice = GenericUser(name= "Alice", laser_port="COM10", PSG_port="COM6", PolCTRL_port1="COM12") # COM4 is LUNA, COM21 is LADAQ+INQNET PolCTRL
    ...
    Charlie.NIDAQ_USB.Alice_power = np.sum(MeasureFunction(Charlie.NIDAQ_USB, [1,2,3,4]))
    ...
    Charlie.NIDAQ_USB.Bob_power = np.sum(MeasureFunction(Charlie.NIDAQ_USB, [1,2,3,4]))
    print(f"Bob power: {Charlie.NIDAQ_USB.Bob_power}")

    while True:
        Charlie.OSW.StatusSet(Charlie.OSW.Alice_Switch, 0)
        Charlie.OSW.StatusSet(Charlie.OSW.Bob_Switch, 0)
        Charlie.NIDAQ_USB.zero_power = MeasureFunction(Charlie.NIDAQ_USB, [1,2,3,4])
